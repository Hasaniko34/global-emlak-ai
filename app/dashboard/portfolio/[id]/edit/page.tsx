  ...
  useEffect(() => {
    const fetchPortfolioDetails = async () => {
      ...
      try {
        ...
        // Mevcut gayrimenkul ID'lerini ayarla
        const propertyIds = portfolio.properties.map((property: Property) => property._id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Portföy güncellenirken bir hata oluştu');
      }
      
      // Başarılı güncelleme sonrası portföy detay sayfasına yönlendir
      router.push(`/dashboard/portfolio/${id}`);
    } catch (err: any) {
      console.error('Portföy güncelleme hatası:', err);
      setError(err.message || 'Portföy güncellenirken bir hata oluştu');
    } finally {
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Portföyü Düzenle</h1>
        ...
        {isLoading ? (
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-6">
              ...
              <p className="text-gray-600 text-sm mb-3">Portföye eklemek veya çıkarmak istediğiniz gayrimenkulleri seçin</p>
            ...
            <div className="flex justify-end">
              ...
              >
                {isSubmitting ? 'Güncelleniyor...' : 'Portföyü Güncelle'}
              </button>
            </div>
          </form>
        )}
